...
public class SVMSemanticFeature {
	static String subTypes[] = {"f-airport", "f-building-grounds", "f-path","f-plant","f-subarea-facility",
			"g-continent","g-county-or-district","g-gpe-cluster","g-nation","g-population-center","g-special","g-state-or-province",
			"l-address","l-boundary","l-celestial","l-land-region-natural","l-region-general","l-region-international","l-water-body",
			"o-commercial","o-educational","o-entertainment","o-government","o-media","o-medical-science","o-non-governmental","o-religious","o-sports",
			"p-group","p-indeterminate","p-individual",
			"v-air","v-land","v-subarea-vehicle","v-underspecified","v-water",
			"w-biological","w-blunt","w-chemical","w-exploding","w-nuclear","w-projectile","w-sharp","w-shooting","w-underspecified",
			"o-other"};
	
	public static String types[] = {"wea" , "veh", "per", "fac", "gpe", "loc", "org"}; 
	
	static String ners[] = {"CARDINAL","DATE","EVENT","FAC","GPE","LAW","LOC","MONEY","NORP","ORDINAL",
			"ORG", "PERCENT","PERSON","PRODUCT","QUANTITY","TIME","WORKOFART","LANGUAGE","OTHER"};
	
	public static void init(){
		Common.loadSemanticDic();
		subTypes2 = new HashMap<String, String[]>();
		String str1[] = {"f-airport", "f-building-grounds", "f-path","f-plant","f-subarea-facility",};
		String str2[] = {"g-continent","g-county-or-district","g-gpe-cluster","g-nation","g-population-center","g-special","g-state-or-province",};
		String str3[] = {"l-address","l-boundary","l-celestial","l-land-region-natural","l-region-general","l-region-international","l-water-body",};
		String str4[] = {"o-commercial","o-educational","o-entertainment","o-government","o-media","o-medical-science","o-non-governmental","o-religious","o-sports",};
		String str5[] = {"p-group","p-indeterminate","p-individual",};
		String str6[] = {"v-air","v-land","v-subarea-vehicle","v-underspecified","v-water",};
		String str7[] = {"w-biological","w-blunt","w-chemical","w-exploding","w-nuclear","w-projectile","w-sharp","w-shooting","w-underspecified",};
		...
		loc_suffix.addAll(Common.getLines("/shared/mlrdir1/disk1/home/yzcchen/workspace/ACL12/src/dict/loc_suffix"));
	}
	...
	public static String semanticFeature(EntityMention em, boolean train, PlainText plainText, int label, ArrayList<Element> eles, String sgmFile) {
...
//		featureGroups.add(lengthFea);
		...
		if(ACECommon.suffixes.contains(em.head.substring(em.head.length()-1))) {
		...
		String semantic = getSemanticSymbol(em, eles, sgmFile);
		ArrayList<Fea> semDicFeas = getSemDicFeas(semantic, train, offset);
		allFeas.addAll(semDicFeas);
		offset += 100000;
		ArrayList<Fea> charBaseFeas = getCharBaseFea(em, train, plainText, offset);
	...
	private static ArrayList<Fea> getSemDicFeas(String semantic, boolean train, int offset) {
	...
	private static String getSemanticSymbol(EntityMention em, ArrayList<Element> eles, String sgmFile) {
		PlainText plainText = ACECommon.getPlainText(sgmFile);
		ArrayList<ParseResult> parseResults = ACECorefCommon.readStanfordParseFile(sgmFile.substring(0,sgmFile.length()-3) + "parse2", plainText);
		int[] position = ACECorefCommon.findParseFilePosition(em, plainText, parseResults);
		em.head = em.head.replace("\n", "");
...
//						System.out.println("LOC: " + head);
	...
	private static ArrayList<Fea> getCharBaseFea(EntityMention em, boolean train, PlainText plainText, int offset) {
		ArrayList<Fea> feas = new ArrayList<Fea>();
		...
		if(em.headStart-2>0) {
			String c = plainText.content.substring(em.headStart-2, em.headStart-1);
		...
		if(em.headStart-1>0) {
			String c = plainText.content.substring(em.headStart-1, em.headStart);
			index = getCharFeaIdx(train, c+"_before");
			if(index!=-1) {
				feas.add(new Fea(index+offset, 0.5));
			}
		}
		if(em.headEnd+1<plainText.content.length()) {
			String c = plainText.content.substring(em.headEnd+1, em.headEnd+2);
			index = getCharFeaIdx(train, c+"_after");
			if(index!=-1) {
				feas.add(new Fea(index+offset, 0.5));
			}
		}
		if(em.headEnd+2<plainText.content.length()) {
			String c = plainText.content.substring(em.headEnd+2, em.headEnd+3);
			index = getCharFeaIdx(train, c+"_after");
			feas.add(new Fea(index+offset, 0.5));
		}
		return feas;
